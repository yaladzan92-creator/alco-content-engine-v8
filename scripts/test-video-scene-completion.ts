...
// TEST 8: 2-scene candidate fails closed (returns empty signature)
console.log('Running Test 8: 2-scene candidate fails closed');
const mockCandidate2Scenes: VideoProductionCandidate = {
  ...mockCandidateA,
  production_details: {
    ...mockCandidateA.production_details,
    scenes: [
...
assert(buildVideoScenePlanSignature(mockCandidate2Scenes) === '', '2-scene candidate fails closed');

// TEST 9: 4-scene candidate fails closed (returns empty signature)
console.log('Running Test 9: 4-scene candidate fails closed');
...
// TEST 10: Malformed scene number fails closed (returns empty signature)
console.log('Running Test 10: Malformed scene number fails closed');
const mockCandidateMalformedNum: VideoProductionCandidate = {
  ...
  production_details: {
    ...
    scenes: [
      ...
      { ...mockCandidateA.production_details.scenes[1], scene_number: 3 as any },
      { ...mockCandidateA.production_details.scenes[2], scene_number: 3 },
    ],
  },
};
assert(buildVideoScenePlanSignature(mockCandidateMalformedNum) === '', 'Gapped scene numbers fail closed');

// TEST 11: No legacy field fallback: missing canonical visual_direction fails closed
console.log('Running Test 11: No legacy field fallback (missing visual_direction fails closed)');
const mockCandidateMissingVisual = JSON.parse(JSON.stringify(mockCandidateA));
delete mockCandidateMissingVisual.production_details.scenes[0].visual_direction;
assert(buildVideoScenePlanSignature(mockCandidateMissingVisual) === '', 'Missing canonical visual_direction returns empty signature');

// TEST 12: No legacy field fallback: missing canonical voiceover fails closed
console.log('Running Test 12: No legacy field fallback (missing voiceover fails closed)');
const mockCandidateMissingVO = JSON.parse(JSON.stringify(mockCandidateA));
delete mockCandidateMissingVO.production_details.scenes[0].voiceover;
assert(buildVideoScenePlanSignature(mockCandidateMissingVO) === '', 'Missing canonical voiceover returns empty signature');
...
console.log('--- ALL 27 TESTS IN PHASE 3D-C1C-D PASSED PERFECTLY ---');
